...
export default function Search({ onGuess, artifacts, disabled }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        // If the user clicks outside of the search bar and suggestions list
  ...
  const handleChange = (e) => {
    ...
    const matches = artifacts.filter(
      // Check if the artifact name contains the input anywhere (case-insensitive)
      (a) => a.name.toLowerCase().includes(value.toLowerCase())
    );
  ...
  const handleButtonClick = () => {
    // Show/hide suggestions when the dropdown button is clicked
    if (showSuggestions) setShowSuggestions(false);
    else setShowSuggestions(true);
  };

  const handleSelect = (artifact) => {
    // Send a guess to GameContainer.jsx, reset the input and hide the suggestions after selecting an artifact set
    onGuess(artifact);
    setInput("");
  ...
  return (
    ...
    >
      <input type="text" name="fake-name" style={{ display: "none" }} />{" "}
      {/* Dummy field to stop safari autofill */}
      <input
        type="password"
        name="fake-password"
        style={{ display: "none" }}
      ...
      <div className="relative w-3/4">
        <input
          ...
          disabled={disabled}
          className="focus:outline-0 p-2.5 bg-neutral-800 rounded w-full"
        />
        {/* Search bar */}
        ...
        {showSuggestions &&
          suggestions.length > 0 && ( // Show the list of suggestions when at least 1 exists and showSuggestions is true
            <ul className="absolute top-full left-0 right-0 max-h-115 overflow-y-auto bg-neutral-700 border border-gray-400 rounded w-full z-10 cursor-pointer">
              {suggestions.map(
                (
                  ...
                  <li
                    ...
                    className="p-4 border border-gray-500 flex flex-row items-center justify-between"
                  >
